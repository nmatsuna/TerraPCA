...
parser.add_argument('-e','--edge_frac',type=float,default=default_edge_frac,help=f'The fraction of the edge part to trim (default: {default_edge_frac:.3f})')
...
if args.trim_edge:
    ...
    left_edge = wmin_set+edge_frac*(wmax_set-wmin_set)
    right_edge = wmax_set-edge_frac*(wmax_set-wmin_set)
    list_reject1.append([wmin_set-1,left_edge])
    list_reject1.append([right_edge,wmax_set+1])
...
for iiter in range(args.niter):
    ...
    valid = (sp_part>fmin)&(sp_part<fmax)&(model_part>fmin)&(model_part<fmax)
    sigma = np.std(dev_part[valid]) if np.any(valid) else np.std(dev_part)
    if args.verbose:
    ...
    new_rejects = clip_parts(waves_part,dev_part,sigma,limit=args.clip_limit,margin=args.clip_margin)
...
valid = (sp_part>fmin)&(sp_part<fmax)&(model_part>fmin)&(model_part<fmax)
sigma = np.std(dev_part[valid]) if np.any(valid) else np.std(dev_part)
print(f'   -> telluric model fitted, sigma={sigma:.4f}', file=sys.stderr)
...
if plot_out is not None:
    ...
    ymin2 = np.min([-0.12, -3*sigma])
    ymax2 = np.max([0.12, 3*sigma])
    ...
    for w0, w1 in tel_abs:
        ...
        axs['dev'].fill_between([w0,w1],0.20*ymin2+0.80*ymax2,0.17*ymin2+0.82*ymax2,color='blue',alpha=0.6)
    for w0, w1 in list_reject2:
        axs['sp'].fill_between([w0,w1],1+0.65*(ymax-1),1+0.72*(ymax-1),color='gray',alpha=0.6)
        axs['dev'].fill_between([w0,w1],0.15*ymin2+0.85*ymax2,0.12*ymin2+0.87*ymax2,color='gray',alpha=0.6)
    for w0, w1 in list_reject1:
        axs['sp'].fill_between([w0,w1],1+0.80*(ymax-1),1+0.87*(ymax-1),color='red',alpha=0.6)
        axs['dev'].fill_between([w0,w1],0.10*ymin2+0.90*ymax2,0.07*ymin2+0.92*ymax2,color='red',alpha=0.6)
